...
*)
let small =
    CsvProvider<"../data/SmallTest.csv", ResolutionFolder=ResolutionFolder>.GetSample ()(* output: 
...
*)
let small2 =
    new CsvProvider<"../data/SmallTest.csv", ResolutionFolder=ResolutionFolder>()(* output: 
...
for row in csv.Rows do
    printfn "%f %d %f" (row.Duration / 1.0<second>) row.Foo (defaultArg row.Column3 1.0)(* output: 
1.000000 2 3.000000
...
for row in titanic1.Rows |> Seq.truncate 10 do
    printfn "%s Class = %d Fare = %g" row.Name row.``Passenger Class`` row.Fare(* output: 
...
*)
let doubleOzone =
    airQuality.Map(fun row -> AirQuality.Row(row.Ozone * 2.0, row.``Solar.R``, row.Wind, row.Temp, row.Month, row.Day))(* output: 
...
let airQualityWithExtraRows = airQuality.Append newRows(* output: 
...
val myRows: CsvProvider<...>.Row list =
  [(1, "a", None); (2, "B", Some 11/15/2023 12:37:56 AM)]
val myCsv: MyCsvType
val it: string = "1,a,
2,B,2023-11-15T00:37:56.7309129+00:00
"*)
(**
